...
import { deleteOfertasRequest, getOfertasRequest } from "../api/ofertas.api";


function OfertasView() {

    const [ofertas, setOfertas] = useState([]);
    const [filterofertas, setFilterOfertas] = useState([]);
    ...
    const cargarTipoPersonas = async()=>{
        try {
            const rp = await getOfertasRequest()
            setOfertas(rp.data)
            setFilterOfertas(rp.data)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const filtrado = (filter)=>{
       if(filter){ 
          setFilterOfertas(filterofertas.filter(producto => producto.nombre.toLowerCase().includes(filter.toLowerCase() ) ) )
        }else{
          setFilterOfertas(ofertas)
        }
    }

    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filterofertas.length;
        setItemOffset(newOffset);
     }

     const borrarProducto = async(id)=>{
        try {
          const rp = await deleteOfertasRequest(id);
          setOfertas(filterofertas.filter(fill => fill.ofertaid != id));
          setFilterOfertas(filterofertas.filter(fill => fill.ofertaid != id))
        } catch (error) {
          console.error(error)
        }
     }
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filterofertas.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filterofertas.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filterofertas])

  return (
    <div className="h-full font-sans bg-cover bg-zinc-950">
            <div className="container mx-auto h-full flex flex-1 justify-center items-center">
                <div className="w-full max-w-xl -mt-48">
                    <div className="py-8"> 

                        <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                            <p className="text-white text-center text-xl font-bold">Ofertas</p>
                        </div>
                        ...
                        <div className="my-2 flex justify-end mr-4 ml-4 p-1 bg-gray-700 rounded sm:flex-row flex-col">
                            ...
                            <div className="block relative ml-20 mt-1"> 
                                <Link to='/ofertas/nuevo' className='px-3 py-1 text-white font-light tracking-wider bg-green-700 hover:bg-green-600 rounded text-lg -ml-10 mr-2'>Agregar</Link>                      
                            </div>
                                                    ...
                                                    <tbody className="text-sm divide-y-2 divide-gray-100">
                                                        {
                                                                currentItems.map((tipo,_index)=>(
                                                                    <tr key={tipo.ofertaid}>
                                                                        ...
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className={tipo.estadogeneral?"text-left bg-green-400 text-gray-50 rounded-md px-2":"text-left bg-red-400 text-gray-50 rounded-md px-2"}>
                                                                                {tipo.estadogeneral?"Activo":"Inactivo"}
                                                                            </div>
                                                                        </td>
                                                                                                                                       
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-right -ml-10">                                                                              
                                                                                <Link to={`/ofertas/edit/${tipo.ofertaid}`} className='px-3 py-1 text-white font-light tracking-wider bg-blue-600 hover:bg-blue-500 rounded text-xs '>Editar</Link>                                                                                                                                                            
                                                                                <button 
                                                                                className='px-3 py-1 text-white font-light tracking-wider bg-red-700 hover:bg-red-600 rounded text-xs ml-1'
                                                                                onClick={()=>borrarProducto(tipo.ofertaid)}
                                                                                >
                                                                                    Eliminar
                                                                                </button>
                                                                            </div>
