...
import { getTipoPersonasRequest } from "../api/tipopersona.api";

    

function TipoPersonaView() {
    ...
    const cargarTipoPersonas = async()=>{
        try {
            const rp = await getTipoPersonasRequest()
            setTipoPersona(rp.data)
            setFilterTipoPersona(rp.data)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const filtrado = (filter)=>{
       if(filter){ 
            setFilterTipoPersona(filterTipoPersona.filter(filterTipoPersona => filterTipoPersona.descripcion.toLowerCase().includes(filter.toLowerCase() ) ) )
  
        }else{
            setFilterTipoPersona(tipoPersona)
        }
    }

    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filterTipoPersona.length;
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filterTipoPersona.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filterTipoPersona.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filterTipoPersona])
                ...
                <div className="w-full max-w-lg -mt-48">
                    <div className="py-8"> 
                        ...
                        <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                            <p className="text-white text-center text-xl font-bold">Tipo de Personas</p>
                        </div>
                        ...
                        <div className="my-2 flex justify-end mr-4 ml-4 p-1 bg-gray-700 rounded sm:flex-row flex-col">
                            ...
                            <div className="block relative ml-20 mt-1"> 
                                <Link to='/tipopersona/nuevo' className='px-3 py-1 text-white font-light tracking-wider bg-green-700 hover:bg-green-600 rounded text-lg -ml-10 mr-2'>Agregar</Link>                      
                            </div>
                                            ...
                                            <div className="overflow-x-auto">
                                                ...
                                                <table className="table-auto w-full">
                                                    ...
                                                    <tbody className="text-sm divide-y-2 divide-gray-100">
                                                        {
                                                                currentItems.map((tipopersona)=>(
                                                                    <tr key={tipopersona.tipopersonaid}>
                                                                        ...
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-right">                                                                              
                                                                                <Link to={`/tipopersona/edit/${tipopersona.tipopersonaid}`} className='px-3 py-1 text-white font-light tracking-wider bg-blue-600 hover:bg-blue-500 rounded text-xs '>Editar</Link>                                                                                                                                                            
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                            ))
...
export default TipoPersonaView;