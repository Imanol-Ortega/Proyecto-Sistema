...
function TipoSubProductoView() {

    const [tipoSubProducto, setTipoSubProducto] = useState([]);
    const [filtertipoSubProducto, setFiltertipoSubProducto] = useState([]);
    ...
    const cargarTipoPersonas = async()=>{
        try {
            ...
            setFiltertipoSubProducto(rp.data)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const filtrado = (filter)=>{
       if(filter){ 
            setFiltertipoSubProducto(filtertipoSubProducto.filter(producto => producto.descripcion.toLowerCase().includes(filter.toLowerCase() ) ) )
        }else{
            setFiltertipoSubProducto(tipoSubProducto)
        }
    }

    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filtertipoSubProducto.length;
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filtertipoSubProducto.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filtertipoSubProducto.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filtertipoSubProducto])
                ...
                <div className="w-full max-w-lg -mt-48">
                    <div className="py-8"> 
                        ...
                        <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                            <p className="text-white text-center text-xl font-bold">Tipo de SubProductos</p>
                        </div>
                        ...
                        <div className="my-2 flex justify-end mr-4 ml-4 p-1 bg-gray-700 rounded sm:flex-row flex-col">
                            ...
                            <div className="block relative ml-20 mt-1"> 
                                <Link to='/tiposubproducto/nuevo' className='px-3 py-1 text-white font-light tracking-wider bg-green-700 hover:bg-green-600 rounded text-lg -ml-10 mr-2'>Agregar</Link>                      
                            </div>
                                        ...
                                        <div className="p-3">
                                            <div className="overflow-x-auto">
                                                    ...
                                                    <tbody className="text-sm divide-y-2 divide-gray-100">
                                                        {
                                                                currentItems.map((tipo,_index)=>(
                                                                    <tr key={tipo.tiposubproductoid}>
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-left">
                                                                                {tipo.tiposubproductoid}
                                                                            </div>
                                                                        </td>
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-left">
                                                                        ...
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-right">                                                                              
                                                                                <Link to={`/tiposubproducto/edit/${tipo.tiposubproductoid}`} className='px-3 py-1 text-white font-light tracking-wider bg-blue-600 hover:bg-blue-500 rounded text-xs '>Editar</Link>                                                                                                                                                            
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                            ))
                                                ...
                                                <div className="w-full flex items-center justify-center">
                                                    {
                                                        filtertipoSubProducto.length == 0 ?  
...
export default TipoSubProductoView