...
function CategoriaView() {
    ...
    const filtrado = (filter)=>{
       if(filter){ 
            setFilterCategoria(filterCategoria.filter(producto => producto.descripcion.toLowerCase().includes(filter.toLowerCase() ) ) )
        }else{
    ...
    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filterCategoria.length;
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filterCategoria.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filterCategoria.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filterCategoria])
                ...
                <div className="w-full max-w-lg -mt-48">
                    <div className="py-8"> 
                        ...
                        <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                            <p className="text-white text-center text-xl font-bold">Categorias</p>
                        </div>
                        ...
                        <div className="my-2 flex justify-end mr-4 ml-4 p-1 bg-gray-700 rounded sm:flex-row flex-col">
                            ...
                            <div className="block relative ml-20 mt-1"> 
                                <Link to='/categoria/nuevo' className='px-3 py-1 text-white font-light tracking-wider bg-green-700 hover:bg-green-600 rounded text-lg -ml-10 mr-2'>Agregar</Link>                      
                            </div>
                                                                    ...
                                                                    <tr key={tipo.categoriaid}>
                                                                        ...
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-right">                                                                              
                                                                                <Link to={`/categoria/edit/${tipo.categoriaid}`} className='px-3 py-1 text-white font-light tracking-wider bg-blue-600 hover:bg-blue-500 rounded text-xs '>Editar</Link>                                                                                                                                                            
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                            ))
