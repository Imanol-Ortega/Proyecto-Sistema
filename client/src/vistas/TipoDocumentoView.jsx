...
function TipoDocumentoView() {
    ...
    const cargarTipoPersonas = async()=>{
        try {
            const rp = await getTipoDocumentosRequest()
            setTipoDocumento(rp.data)
            setFilterTipoDocumento(rp.data)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const filtrado = (filter)=>{
       if(filter){ 
            setFilterTipoDocumento(filterTipoDocumento.filter(producto => producto.descripcion.toLowerCase().includes(filter.toLowerCase() ) ) )
        }else{
            setFilterTipoDocumento(tipoDocumento)
        }
    }

    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filterTipoDocumento.length;
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filterTipoDocumento.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filterTipoDocumento.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filterTipoDocumento])
  return (
                ...
                <div className="w-full max-w-lg -mt-48">
                    <div className="py-8"> 
                        ...
                        <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                            <p className="text-white text-center text-xl font-bold">Tipo de Documento</p>
                        </div>
                                                                    ...
                                                                    <tr key={tipo.tipodocumentoid}>
                                                                        ...
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-right">                                                                              
                                                                                <Link to={`/tipodocumento/edit/${tipo.tipodocumentoid}`} className='px-3 py-1 text-white font-light tracking-wider bg-blue-600 hover:bg-blue-500 rounded text-xs '>Editar</Link>                                                                                                                                                            
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                            ))
...
export default TipoDocumentoView