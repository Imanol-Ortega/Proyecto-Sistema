...
function SubProductoView() {
    ...
    const [filtersubproducto, setFilterSubProducto] = useState([]);
    ...
    const cargarSubproducto = async()=>{
        try {
            const rp = await getSubProductosRequest();
            setSubProducto(rp.data)
            setFilterSubProducto(rp.data)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const filtrado = (filter)=>{
       if(filter){ 
            setFilterSubProducto(filtersubproducto.filter(producto => producto.nombre.toString().toLowerCase().includes(filter.toString().toLowerCase())))
        }else{
            setFilterSubProducto(subproducto)
        }
    }


    const borrarSubproducto = async(id)=>{
        try {
            const rp = await dltSubProductoRequest(id);
            setSubProducto(filtersubproducto.filter(filcompra => filcompra.subproductoid !=id))
            setFilterSubProducto(filtersubproducto.filter(filinventario => filinventario.subproductoid !=id))
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filtersubproducto.length;
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filtersubproducto.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filtersubproducto.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filtersubproducto])
        ...
        <div className="w-full max-w-4xl -mt-48">
            <div className="py-8"> 
                ...
                <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                    <p className="text-white text-center text-xl font-bold">Sub Productos</p>
                </div>
                ...
                <div className="my-2 flex justify-end mr-4 ml-4 p-1 bg-gray-700 rounded sm:flex-row flex-col">
                    ...
                    <div className="block relative ml-20 mt-1">
                        <Link to='/subproducto/nuevo' className='px-3 py-1 text-white font-light tracking-wider bg-green-700 hover:bg-green-600 rounded text-lg -ml-10 mr-2'>Agregar</Link>                      
                    </div>
...
export default SubProductoView