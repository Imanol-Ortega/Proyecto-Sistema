...
import { dltProductoRequest, getProductosRequest } from "../api/productos.api";
...
function ProductosView() {
    ...
    const filtrado = (filter)=>{
       if(filter){ 
          setFilterProductos(filterproductos.filter(producto => producto.nombre.toLowerCase().includes(filter.toLowerCase() ) ) )
        }else{
    ...
    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filterproductos.length;
        setItemOffset(newOffset);
     }

     const borrarProducto = async(id)=>{
        try {
          const rp = await dltProductoRequest(id);
          setProductos(filterproductos.filter(fill => fill.productoid != id));
          setFilterProductos(filterproductos.filter(fill => fill.productoid != id))
        } catch (error) {
          console.error(error)
        }
     }
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filterproductos.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filterproductos.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filterproductos])
  return (
                ...
                <div className="w-full max-w-6xl -mt-48">
                    <div className="py-8"> 
                        ...
                        <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                            <p className="text-white text-center text-xl font-bold">Productos</p>
                        </div>
                        ...
                        <div className="my-2 flex justify-end mr-4 ml-4 p-1 bg-gray-700 rounded sm:flex-row flex-col">
                            ...
                            <div className="block relative ml-20 mt-1"> 
                                <Link to='/productos/nuevo' className='px-3 py-1 text-white font-light tracking-wider bg-green-700 hover:bg-green-600 rounded text-lg -ml-10 mr-2'>Agregar</Link>                      
                            </div>
                                                                    ...
                                                                    <tr key={tipo.productoid}>
                                                                        ...
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-right -ml-10">                                                                              
                                                                                <Link to={`/productos/edit/${tipo.productoid}`} className='px-3 py-1 text-white font-light tracking-wider bg-blue-600 hover:bg-blue-500 rounded text-xs '>Editar</Link>                                                                                                                                                            
                                                                                <button 
                                                                                className='px-3 py-1 text-white font-light tracking-wider bg-red-700 hover:bg-red-600 rounded text-xs ml-1'
                                                                                onClick={()=>borrarProducto(tipo.productoid)}
                                                                                >
                                                                                    Eliminar
                                                                                </button>
                                                                            </div>
