...
function TipoProductoView() {
    ...
    const cargarTipoPersonas = async()=>{
        try {
            ...
            setTipoProducto(rp.data)
            setFilterTipoProducto(rp.data)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const filtrado = (filter)=>{
       if(filter){ 
            setFilterTipoProducto(filterTipoProducto.filter(producto => producto.descripcion.toLowerCase().includes(filter.toLowerCase() ) ) )
        }else{
            setFilterTipoProducto(tipoProducto)
        }
    }

    const handlePageClick = (e)=>{
        const newOffset = (e.selected * itemsPerPage) % filterTipoProducto.length;
    ...
    useEffect(()=>{
        ...
        setCurrentItems(filterTipoProducto.slice(itemOffset,endOffset));
        setPageCount(Math.ceil(filterTipoProducto.length / itemsPerPage));
    },[itemOffset,itemsPerPage,filterTipoProducto])
  return (
                ...
                <div className="w-full max-w-lg -mt-48">
                    <div className="py-8"> 
                        ...
                        <div className="my-2 flex justify-center mr-4 ml-4 p-1 sm:flex-row flex-col">
                            <p className="text-white text-center text-xl font-bold">Tipo de Producto</p>
                        </div>
                                                                    ...
                                                                    <tr key={tipo.tipoproductoid}>
                                                                        ...
                                                                        <td className="p-2 whitespace-nowrap">
                                                                            <div className="text-right">                                                                              
                                                                                <Link to={`/tipoproducto/edit/${tipo.tipoproductoid}`} className='px-3 py-1 text-white font-light tracking-wider bg-blue-600 hover:bg-blue-500 rounded text-xs '>Editar</Link>                                                                                                                                                            
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                            ))
