...
export const getProveedores = async(req,res)=>{
    try {
        const response = await pool.query('SELECT * FROM proveedores WHERE activo = TRUE');
        res.json(response.rows)
    } catch (error) {
        return res.status(500).json({message:error.message});
    }
...
export const dltProveedores = async(req,res)=>{
    try {
        const response = await pool.query('UPDATE proveedores SET activo = FALSE WHERE proveedorid = $1',[req.params.id]);
        res.json(response)
    } catch (error) {
        return res.status(500).json({message:error.message});
    }
